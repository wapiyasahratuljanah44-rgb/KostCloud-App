...
<div class="max-w-xl bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
    <h3 class="text-lg font-bold text-slate-900 mb-4">Registrasi Penyewa Baru</h3>
    ...
    <form action="{{ route('tenants.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name') }}" required class="w-full p-3 border border-slate-200 rounded-xl text-sm focus:outline-blue-500 bg-slate-50">
        </div>
        <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1">No. WhatsApp/Phone</label>
            <input type="text" name="phone" value="{{ old('phone') }}" placeholder="08xxxx" required class="w-full p-3 border border-slate-200 rounded-xl text-sm focus:outline-blue-500 bg-slate-50">
        </div>
        <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1">Alamat Email</label>
            <input type="email" name="email" value="{{ old('email') }}" required class="w-full p-3 border border-slate-200 rounded-xl text-sm focus:outline-blue-500 bg-slate-50">
        </div>
